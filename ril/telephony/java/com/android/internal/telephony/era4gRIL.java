...
public class era4gRIL extends RIL {
    ...
    public era4gRIL(Context context, int preferredNetworkType, int cdmaSubscription) {
        super(context, preferredNetworkType, cdmaSubscription, null);
    }

    public era4gRIL(Context context, int preferredNetworkType,
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
        ...
        if (rr.mRequest == RIL_REQUEST_SET_PREFERRED_NETWORK_TYPE) {
            if (mPreferredNetworkType < 8 || mPreferredNetworkType > 12) {
                Settings.System.putInt(mContext.getContentResolver(), "lte_mode_switch", 0);
                Rlog.d("RILJ", "Set LTE_MODE_SWITCH off");
            } else {
                Settings.System.putInt(mContext.getContentResolver(), "lte_mode_switch", 1);
    ...
    processUnsolicited(Parcel p) {
        ...
        switch (response) {
            case RIL_UNSOL_AM:
                ...
                Rlog.d(RILJ_LOG_TAG, "Executing Am " + strAm);
                break;
        }
    }
}
